...
// web app uses. To support long meetings, each track is recorded as a series of
// ~5-minute SEGMENTS (seq 0,1,2,…) — the transcription worker concatenates them —
// so no single uploaded file ever hits the storage/edge size ceiling. Recordings
// are always saved locally first, so a failed upload is retryable and never lost.
'use strict'
...
function setCaptureProfile(profile, { syncSource = true, check = true } = {}) {
  ...
  $('mode-hint').textContent = room
    ...
    : 'Captures your microphone and, when enabled, the call audio as separate tracks.'
...
// A pre-record device check: is the mic live (with a level meter to prove it), is a
// call-audio output device present, and is there enough disk for a long meeting? Purely
// informational + reassuring — Start still works; capture errors also surface on Start.
...
async function runPreflight() {
  ...
  setPreflightRow('output', 'checking', 'Call audio output', 'Checking…')
  ...
  try {
    ...
    setPreflightRow('mic', 'fail', 'Microphone', `Not available (${e.name || e.message}). Allow microphone access in Windows Settings → Privacy → Microphone, then re-check.`)
  }

  if (captureProfile() === 'room_single_mic') {
    setPreflightRow('output', 'ok', 'Recording setup', 'Room microphone only — system audio is not needed.')
  } else {
    // Output device: needed so system/call audio can be captured (loopback) at Start.
    try {
      const outs = (await navigator.mediaDevices.enumerateDevices()).filter((d) => d.kind === 'audiooutput')
      if (outs.length) setPreflightRow('output', 'ok', 'Call audio output', 'Ready — make sure the call plays through your DEFAULT Windows output device.')
      else setPreflightRow('output', 'warn', 'Call audio output', 'No output device detected — call audio may not be captured.')
    } catch {
      setPreflightRow('output', 'warn', 'Call audio output', 'Could not check output devices.')
    }
  }
...
async function startRecording() {
  ...
  const wantSystem = profile === 'remote_dual_track' && $('system').checked
